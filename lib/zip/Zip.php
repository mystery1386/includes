...
class Zip {
     ...
     * @throws Exception
     */
    function __construct($useZipFile = FALSE, $compression_level=-1) {
        if ($useZipFile) {
        ...
        if ($compression_level >= -1 && $compression_level <= 9) {
            ...
            throw new Exception("Invalid compression level: {$compression_level}, use -1 to 9.");
        }
    }
    ...
    public function addFile($data, $filePath, $timestamp = 0, $fileComment = NULL, $compress = TRUE) {
        ...
        if ($compress) {
            $gzTmp = gzcompress($data, $this->compressionLevel);
